...
OUTFILE=variable_blk.suncc-misses.matsize-blksize-memfoot-mflops-errs.${PBS_JOBID}.txt
#PBS -o $OUTFILE
...
do
      ...
      # experiment name
      JID=`echo ${PBS_JOBID%.*}`
      EXPOUT="$PBS_JOBNAME.${PERM}.${size}.${bs}.${JID}.er"
...
# start the collect command with the above settings
        #printf "${size} ${BLKSIZE} " >> $OUTFILE
        #MATMULT_COMPARE=0 /usr/bin/time -f'%E' ./$EXECUTABLE $PERM $MKN $BLKSIZE >> $OUTFILE 2>&1
        MATMULT_COMPARE=0 collect -o $EXPOUT $HWCOUNT ./$EXECUTABLE $PERM $MKN $BLKSIZE
      done
    done
done
